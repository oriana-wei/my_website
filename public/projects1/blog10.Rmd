...
keywords: ""
slug: blog10
title: Climate Change and Temperature Anomalies Analysis
---
...
Let's have a look on the dataset we have first:
```{r}
glimpse(weather)
```
...
```{r scatter_plot,fig.width=22, fig.height= 20}
...
```{r facet_wrap,fig.width=22, fig.height= 20}
...
```{r density_plot, eval=FALSE,fig.width=22, fig.height= 20}
## fix the colors 
...
```{r averaging, eval=FALSE,fig.width=22, fig.height= 20}
...
Here comes our visulisation:
```{r, calculate_CI_using_bootstrap, fig.length="100%",fig.width=22, fig.height= 20}
